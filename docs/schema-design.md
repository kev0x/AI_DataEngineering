...
```text
Source parsing belongs in ETL code.
Business classification belongs in Silver.mapCategoryRule data.
```

`ProcessFactTransaction.sql` should not grow merchant-specific `case` logic. Known patterns
such as investment transfers, credit-card payments, outgoing peer payments, manual user
corrections, and future AI suggestions should be represented as `Silver.mapCategoryRule`
rows. The fact ETL applies the highest-priority active matching rule and stores the
winning `categoryRuleKey` for traceability.

Rule source conventions:

```text
rule    system-seeded rules
manual  user-approved rules from the dashboard/API
ai      future AI-suggested rules approved into the warehouse
```
...
`Gold.vw_TransactionLedger` is the safe transaction-level view for the React dashboard.
It exposes dates, account type, merchant display name, category, event type, and amount,
but it does not expose raw Chase descriptions, memo, balance, account last four, check/slip
number, or source file names.
...
- Surrogate keys are generated by table defaults backed by DuckDB sequences.
...
- Code files that support comments include a purpose/dependencies header.
